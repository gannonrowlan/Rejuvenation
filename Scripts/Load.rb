...
class PokemonLoad
  ...
  def startPlayingSaveFile(savefile)
    ...
    guaranteed_items        = [
      :ADAMANTMINT,:LONELYMINT,:NAUGHTYMINT,:BRAVEMINT,
      ...
      :SERIOUSMINT,:ABILITYCAPSULE,:ABILITYPATCH,:BLUEMIC,:LINKHEART
    ]
    guaranteed_items.each do |item_symbol|
      next if !$cache || !$cache.items || !$cache.items[item_symbol]
      current_item_count = $PokemonBag.pbQuantity(item_symbol)
      if current_item_count < desired_item_count
        $PokemonBag.pbStoreItem(item_symbol,desired_item_count-current_item_count)
      end
    end
    desired_training_item_count = 999
    training_items = [
      ...
      :HPRESETBAG,:ATKRESETBAG,:DEFRESETBAG,:SPARESETBAG,:SPDRESETBAG,:SPERESETBAG,:FULLRESETBAG,
      :CELLIMPRINT,:NEGATIVEIMPRINT
    ]
    training_items.each do |item_symbol|
      next if !$cache || !$cache.items || !$cache.items[item_symbol]
      current_item_count = $PokemonBag.pbQuantity(item_symbol)
      if current_item_count < desired_training_item_count
        $PokemonBag.pbStoreItem(item_symbol,desired_training_item_count-current_item_count)
      end
    end
