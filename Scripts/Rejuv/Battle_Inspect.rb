PokeBattle_Battler.class_eval{
  ...
  def pbCalcSpAtk()
    ...
    if (self.ability == (:PLUS) || self.ability == (:MINUS))
      ...
      elsif @battle.FE == :SHORTCIRCUIT || @battle.FE == :ELECTERRAIN || @battle.state.effects[:ELECTERRAIN] > 0
        atkmult=(atkmult*1.5).round
      end
    end
    ...
    atkmult=(atkmult*1.5).round if self.crested == :CASTFORM && self.form == 1 && @battle.pbWeather== :SUNNYDAY
